...
** 6       06/08/2018   Vikram Haibate		 Split Ala-Carte product holding - beta4 
***************************************************************************************************************************************************************

-----------------------------------------------------------------------DDLS------------------------------------------------------------------------------------
dba.sp_drop_table 'Decisioning','ECONOMETRICS_TA_CALLS_beta6'
dba.sp_create_table 'Decisioning','ECONOMETRICS_TA_CALLS_beta6', 
...
dba.sp_drop_table 'Decisioning','ECONOMETRICS_TA_CALLS_OFFERS_beta6'
dba.sp_create_table 'Decisioning','ECONOMETRICS_TA_CALLS_OFFERS_beta6', 
   ' YEAR_WEEK VARCHAR (31) DEFAULT NULL, '
|| ' WEEK_START DATE  DEFAULT NULL, '
|| ' COUNTRY VARCHAR (16) DEFAULT NULL, '
|| ' SAVED_FLAG SMALLINT  DEFAULT 0, '
...
dba.sp_drop_table 'Decisioning','ECONOMETRICS_TA_CALLS_OFFERS_ENDING_beta6'
dba.sp_create_table 'Decisioning','ECONOMETRICS_TA_CALLS_OFFERS_ENDING_beta6', 
   ' YEAR_WEEK VARCHAR (31) DEFAULT NULL, '
|| ' WEEK_START DATE DEFAULT NULL,'
|| ' COUNTRY VARCHAR (16) DEFAULT NULL,'
|| ' SAVED_FLAG SMALLINT DEFAULT 0,'
...
dba.sp_drop_table 'Decisioning','ECONOMETRICS_TA_CALLS_OFFERS_ACTIVE_beta6'
dba.sp_create_table 'Decisioning','ECONOMETRICS_TA_CALLS_OFFERS_ACTIVE_beta6', 
   ' YEAR_WEEK VARCHAR (31) DEFAULT NULL,'
|| ' WEEK_START DATE  DEFAULT NULL,'
|| ' COUNTRY VARCHAR (16) DEFAULT NULL,'
|| ' SAVED_FLAG SMALLINT  DEFAULT 0,'
...
dba.sp_drop_table 'Decisioning','ECONOMETRICS_TA_CALLS_ACCOUNT_LEVEL_beta6'
dba.sp_create_table 'Decisioning','ECONOMETRICS_TA_CALLS_ACCOUNT_LEVEL_beta6', 
...
--GO
DROP PROCEDURE IF EXISTS vha02.Update_Econometrics_TA_Events_beta;
--GO
create procedure vha02.Update_Econometrics_TA_Events_beta( PERIOD_START date default null,PERIOD_END date default null ) 
sql security invoker
begin

  set option Query_Temp_Space_Limit = 0;
  if PERIOD_START is null then
    set PERIOD_START = (select max(week_start)-4*7 from Decisioning.ECONOMETRICS_TA_CALLS_beta6)
  end if;
 ...
 select aa.account_number,aa.event_Dt,aa.subscription_sub_type,aa.TA_attempt_type,max(bb.event_dt) as last_ta_dt,datediff(month,max(bb.event_dt),aa.event_dt) as Time_Since_Last_TA_Event
  ...
  select A.ACCOUNT_NUMBER,
    ...
    end as turnaround_description,case when A.TA_attempt_type = 'Web Chat' then 1 else 0 end as livechat_turnaround_event,
 ...
 update #TA_CALLS2 as base
  ...
  update #TA_CALLS2 as base
   set base.pre_call_Spotify = asr.Spotify_active_subscription
    from
    #TA_CALLS2 as base
    join CITeam.active_subscriber_report as asr
    on base.account_number = asr.account_number
...
-- contract 
  ...
  update #TA_CALLS2 as base
    set base.Basic_Contract_Status_Level_2 = case when ctr.contract_status = 'In Contract' then
      ...
      when ctr.status_end_date > (EVENT_DT+56) then 'Contract Ending in 9+ Wks'
      else 'No Contract'
  ...
  update #TA_CALLS2 as base
    set base.Add_on_Products_Contract_Status_Level_2 = case when ctr.contract_status = 'In Contract' then
      case when ctr.status_end_date between EVENT_DT and(EVENT_DT+7) then 'Contract Ending in Next 1 Wks'
      when ctr.status_end_date between(EVENT_DT+8) and(EVENT_DT+14) then 'Contract Ending in Next 2-3 Wks'
      when ctr.status_end_date between(EVENT_DT+15) and(EVENT_DT+21) then 'Contract Ending in Next 2-3 Wks'
      when ctr.status_end_date between(EVENT_DT+22) and(EVENT_DT+28) then 'Contract Ending in Next 4-6 Wks'
  ...
  update #TA_CALLS2
    ...
    'Contract Ended in last 2-3 Wks','Contract Ended in last 4-6 Wks','Contract Ended in last 7-8 Wks' ) then
      'Contract End'
    when BB_Contract_Status_Level_2 = 'Contract Ending in 9+ Wks' then
      'In-Contract'
    else 'No Contract'
    end;
  update #TA_CALLS2
    ...
    'Contract Ended in last 2-3 Wks','Contract Ended in last 4-6 Wks','Contract Ended in last 7-8 Wks' ) then
      'Contract End'
    when Talk_Contract_Status_Level_2 = 'Contract Ending in 9+ Wks' then
      'In-Contract'
    else 'No Contract'
    end;
  update #TA_CALLS2
    ...
    'Contract Ended in last 2-3 Wks','Contract Ended in last 4-6 Wks','Contract Ended in last 7-8 Wks' ) then
      'Contract End'
    when Add_on_Products_Contract_Status_Level_2 = 'Contract Ending in 9+ Wks' then
      'In-Contract'
    else 'No Contract'
    end;
  update #TA_CALLS2
    ...
    'Contract Ended in last 2-3 Wks','Contract Ended in last 4-6 Wks','Contract Ended in last 7-8 Wks' ) then
      'Contract End'
    when Basic_Contract_Status_Level_2 = 'Contract Ending in 9+ Wks' then
      'In-Contract'
    else 'No Contract'
    end;
...
-- ASR Pivot start 
  ...
  update #ASR_ECON as base
    ...
    hd_basic = case when asr.CURRENT_PRODUCT_SK in( 43678,53103 ) then 1 else 0 end
   from  #ASR_ECON as base
    join CITeam.active_subscriber_report as asr
    on base.account_number = asr.account_number
...
-- end of pivot 	
  ...
  select A.*,
    ...
    case when B.bb_product_holding in( 'Fibre Max',
    'Sky Fibre',
    'Fibre Lite',
    'Fibre (ROI - Legacy)',
    'Fibre (ROI)' ) then
      1 else 0 end as PRE_CALL_BB_FIBRE_CAP,
    case when B.bb_product_holding in( 'Unlimited Fibre',
    'Fibre Unlimited (ROI - Legacy)',
    'Fibre Unlimited (ROI)',
    'Fibre Unlimited Pro',
    'Fibre Unlimited Plus' ) then
      1 else 0 end as PRE_CALL_BB_FIBRE_UNLIMITED,
    case when B.bb_product_holding in( 'Sky Broadband Lite (ROI)',
    'Sky Broadband Lite',
    'Connect Lite (ROI)',
    'Connect Lite (ROI - Legacy)' ) then
      1 else 0 end as PRE_CALL_BB_LITE,
    case when B.bb_product_holding in( 'Unlimited (Legacy)',
    'Unlimited',
    'Unlimited (ROI - Legacy)',
    'Unlimited Pro',
    'Connect Unlimited (ROI - Legacy)',
    'Unlimited (ROI)',
    'Connect Unlimited (ROI)','Connect','Everyday' ) then
      1 else 0 end as PRE_CALL_BB_UNLIMITED,
    case when(PRE_CALL_BB_UNLIMITED+A.BB_UNLIMITED_ADDED-A.BB_UNLIMITED_REMOVED) > 0 then 1 else 0 end as POST_CALL_BB_UNLIMITED,
    case when(PRE_CALL_BB_LITE+A.BB_LITE_ADDED-A.BB_LITE_REMOVED) > 0 then 1 else 0 end as POST_CALL_BB_LITE,
    case when(PRE_CALL_BB_FIBRE_CAP+A.BB_FIBRE_CAP_ADDED-A.BB_FIBRE_CAP_REMOVED) > 0 then 1 else 0 end as POST_CALL_BB_FIBRE_CAP,
    case when(PRE_CALL_BB_FIBRE_UNLIMITED+A.BB_FIBRE_UNLIMITED_ADDED-A.BB_FIBRE_UNLIMITED_REMOVED) > 0 then 1 else 0 end as POST_CALL_BB_FIBRE_UNLIMITED,
	...
	cast(null as varchar(30)) as Movies_OFFER_END_STATUS_LEVEL_1,
	...
	cast(null as varchar(30)) as Movies_OFFER_END_STATUS_LEVEL_2,
...
-- Product holding 
    ...
    set base.PRE_CALL_SPORTS_PRODUCT_HOLDING = asr.product_holding from
    #TA_CALLS3 as base
    join CITeam.active_subscriber_report as asr
    on base.account_number = asr.account_number
    where base.EVENT_DT_SoD between asr.effective_from_dt and asr.effective_to_dt-1
	...
	update #TA_CALLS3 as base
    set base.POST_CALL_SPORTS_PRODUCT_HOLDING = asr.product_holding from
    #TA_CALLS3 as base
    join CITeam.active_subscriber_report as asr
    on base.account_number = asr.account_number
    where base.EVENT_DT between asr.effective_from_dt and asr.effective_to_dt-1
 ...
 update #TA_CALLS3 as base
  ...
  call vha02.Add_Active_Subscriber_Product_Holding('#TA_CALLS3','EVENT_DT_SoD','Sports','Update Only','Sports_Active','Sports_Product_Count');
  call vha02.Add_Active_Subscriber_Product_Holding('#TA_CALLS3','EVENT_DT_SoD','Movies','Update Only','Movies_Active','Movies_Product_Count');
  call vha02.Add_Active_Subscriber_Product_Holding('#TA_CALLS3','EVENT_DT_SoD','Prems','Update Only','Prems_Active','Prems_Product_Count');
  call vha02.Add_Active_Subscriber_Product_Holding('#TA_CALLS3','EVENT_DT_SoD','BB','Update Only','BB_ACTIVE','BB_PRODUCT_HOLDING');
  update #TA_CALLS3
  ...
  call vha02.Add_Active_Subscriber_Product_Holding('#TA_CALLS3','EVENT_DT','BB','Update Only','BB_ACTIVE','BB_PRODUCT_HOLDING');
  update #TA_CALLS3
  ...
  commit work;
  call vha02.Add_Active_Subscriber_Product_Holding('#TA_CALLS3','event_Dt','Sports','Update Only','Sports_Active','Sports_Product_Count');
  call vha02.Add_Active_Subscriber_Product_Holding('#TA_CALLS3','event_Dt','Movies','Update Only','Movies_Active','Movies_Product_Count');
  update #TA_CALLS3
...
-- Offers status  start 	
call Decisioning_Procs.Add_Offers_Software('#TA_CALLS3','EVENT_DT_SoD','DTV','Ordered','All','Update Only','Prev_Offer_Actual_End_Dt_DTV','Curr_Offer_Intended_end_Dt_DTV');
  
  Update #TA_CALLS3 
  Set Basic_Offer_End_Status_Level_2 = 
        case when PRE_CALL_DTV = 1 then 
		(case 
		     when Curr_Offer_Intended_end_Dt_DTV between (EVENT_DT_SoD + 1) and (EVENT_DT_SoD + 7)   then 'Offer Ending in Next 1 Wks'
...
call Decisioning_Procs.Add_Offers_Software('#TA_CALLS3','EVENT_DT_SoD','BROADBAND','Ordered','All','Update Only','Prev_Offer_Actual_End_Dt_BB','Curr_Offer_Intended_end_Dt_BB');
...
call Decisioning_Procs.Add_Offers_Software('#TA_CALLS3','EVENT_DT_SoD','LR','Ordered','All','Update Only',  'Prev_Offer_Actual_End_Dt_LR','Curr_Offer_Intended_end_Dt_LR');
...
call Decisioning_Procs.Add_Offers_Software('#TA_CALLS3','EVENT_DT_SoD','Sports','Ordered','All','Update Only','Prev_Offer_Actual_End_Dt_Sports','Curr_Offer_Intended_end_Dt_Sports');
...
call Decisioning_Procs.Add_Offers_Software('#TA_CALLS3','EVENT_DT_SoD','Movies','Ordered','All','Update Only','Prev_Offer_Actual_End_Dt_Movies','Curr_Offer_Intended_end_Dt_Movies');


Update #TA_CALLS3 
Set Movies_OFFER_END_STATUS_LEVEL_2 = 
		case when PRE_CALL_TOTAL_PREMIUMS>0 then 
...
update #TA_CALLS3
	set Movies_OFFER_END_STATUS_LEVEL_1
              = case when Movies_OFFER_END_STATUS_LEVEL_2 in( 'Offer Ending in Next 1 Wks','Offer Ending in Next 2-3 Wks','Offer Ending in Next 4-6 Wks','Offer Ended in last 1 Wks','Offer Ended in last 2-3 Wks','Offer Ended in last 4-6 Wks' ) then
					'Offer Ending'
					 when Movies_OFFER_END_STATUS_LEVEL_2 = 'Offer Ending in 7+ Wks' then 'On Offer'
				else 'No Offer'
end;
...
update #TA_CALLS3
    set PREMIUM_Offer_End_Status_Level_1
     =  	case when PRE_CALL_TOTAL_PREMIUMS>0 then (
	        case when Sports_OFFER_END_STATUS_LEVEL_1 = 'Offer Ending' or Movies_OFFER_END_STATUS_LEVEL_1 = 'Offer Ending' then  'Offer Ending'
			     when Sports_OFFER_END_STATUS_LEVEL_1 = 'On Offer' or Movies_OFFER_END_STATUS_LEVEL_1 = 'On Offer'  then 'On Offer'
			 else 'No Offer'
	    end )
		 else 'No Offer' end; 
...
Set PREMIUM_Offer_End_Status_Level_2 = 
        ...
        (  
		   case when PREMIUM_Offer_End_Status_Level_1 = 'No Offer' then 'No Offer'
				...
				when PREMIUM_Offer_End_Status_Level_1 = 'Offer Ending' 
														then (
														case 
																when Sports_OFFER_END_STATUS_LEVEL_2 in( 'Offer Ending in Next 1 Wks') or Movies_OFFER_END_STATUS_LEVEL_2 in( 'Offer Ending in Next 1 Wks')  then  'Offer Ending in Next 1 Wks'
																when Sports_OFFER_END_STATUS_LEVEL_2 in('Offer Ending in Next 2-3 Wks') or Movies_OFFER_END_STATUS_LEVEL_2 in('Offer Ending in Next 2-3 Wks')  then  'Offer Ending in Next 2-3 Wks'
																when Sports_OFFER_END_STATUS_LEVEL_2 in('Offer Ending in Next 4-6 Wks') or Movies_OFFER_END_STATUS_LEVEL_2 in('Offer Ending in Next 4-6 Wks')  then  'Offer Ending in Next 4-6 Wks'
																when Sports_OFFER_END_STATUS_LEVEL_2 in('Offer Ended in last 1 Wks') or Movies_OFFER_END_STATUS_LEVEL_2 in('Offer Ended in last 1 Wks')  then  'Offer Ended in last 1 Wks'
																when Sports_OFFER_END_STATUS_LEVEL_2 in('Offer Ended in last 2-3 Wks') or Movies_OFFER_END_STATUS_LEVEL_2 in('Offer Ended in last 2-3 Wks')  then  'Offer Ended in last 2-3 Wks'
																when Sports_OFFER_END_STATUS_LEVEL_2 in('Offer Ended in last 4-6 Wks') or Movies_OFFER_END_STATUS_LEVEL_2 in('Offer Ended in last 4-6 Wks')  then  'Offer Ended in last 4-6 Wks'
...
-- ADD On Packages start 		

-- HD  

call Decisioning_Procs.Add_Offers_Software('#TA_CALLS3','EVENT_DT_SoD','HD','Ordered','All','Update Only','Prev_Offer_Actual_End_Dt_HD','Curr_Offer_Intended_end_Dt_HD');
...
-- ms 
call Decisioning_Procs.Add_Offers_Software('#TA_CALLS3','EVENT_DT_SoD','MS','Ordered','All','Update Only','Prev_Offer_Actual_End_Dt_MS','Curr_Offer_Intended_end_Dt_MS');
...
-- SGE 
call Decisioning_Procs.Add_Offers_Software('#TA_CALLS3','EVENT_DT_SoD','SGE','Ordered','All','Update Only','Prev_Offer_Actual_End_Dt_SGE','Curr_Offer_Intended_end_Dt_SGE');
...
call Decisioning_Procs.Add_Offers_Software('#TA_CALLS3','EVENT_DT_SoD','HD PACK','Ordered','All','Update Only','Prev_Offer_Actual_End_Dt_HD_PACK','Curr_Offer_Intended_end_Dt_HD_PACK');
...
call Decisioning_Procs.Add_Offers_Software('#TA_CALLS3','EVENT_DT_SoD','BOX SETS','Ordered','All','Update Only','Prev_Offer_Actual_End_Dt_BOX_SETS','Curr_Offer_Intended_end_Dt_BOX_SETS');
...
call Decisioning_Procs.Add_Offers_Software('#TA_CALLS3','EVENT_DT_SoD','SKY_KIDS','Ordered','All','Update Only','Prev_Offer_Actual_End_Dt_SKY_KIDS','Curr_Offer_Intended_end_Dt_SKY_KIDS');
...
call Decisioning_Procs.Add_Offers_Software('#TA_CALLS3','EVENT_DT_SoD','TALK','Ordered','All','Update Only','Prev_Offer_Actual_End_Dt_TALK','Curr_Offer_Intended_end_Dt_TALK');
...
call Decisioning_Procs.Add_Offers_Software('#TA_CALLS3','EVENT_DT_SoD','SPOTIFY','Ordered','All','Update Only','Prev_Offer_Actual_End_Dt_SPOTIFY','Curr_Offer_Intended_end_Dt_SPOTIFY');
...
Set ADD_ON_PRODUCTS_OFFER_END_STATUS_LEVEL_2 = 
        ...
        (  
		   case when ADD_ON_PRODUCTS_OFFER_END_STATUS_LEVEL_1 = 'No Offer' then 'No Offer'
				...
				when ADD_ON_PRODUCTS_OFFER_END_STATUS_LEVEL_1 = 'Offer Ending' 
														then (
														case 
...
Set Offer_End_Status_Level_2 = 
        ...
        (  
		   case when Offer_End_Status_Level_1 = 'No Offer' then 'No Offer'
				...
				when Offer_End_Status_Level_1 = 'Offer Ending' 
														then (
														case 
...
-- end 
 call Decisioning_Procs.Add_Offers_Software('#TA_CALLS3','EVENT_DT','ANY','Ordered','New','Update Only','Offers_Applied_Lst_1D_Any');
  call Decisioning_Procs.Add_Offers_Software('#TA_CALLS3','EVENT_DT','DTV','Ordered','New','Update Only','Offers_Applied_Lst_1D_DTV');
  update #TA_CALLS3
  ...
  call Decisioning_Procs.Add_Offers_Software('#TA_CALLS3','EVENT_DT_SoD','ANY','Ordered','All','Update Only','Offers_Applied_Lst_day_any');
  call Decisioning_Procs.Add_Offers_Software('#TA_CALLS3','EVENT_DT_SoD','DTV','Ordered','All','Update Only','Offers_Applied_Lst_day_dtv');
  call Decisioning_Procs.Add_Offers_Software('#TA_CALLS3','EVENT_DT','ANY','Ordered','All','Update Only','Offers_Applied_Lst_1D_Any');
  call Decisioning_Procs.Add_Offers_Software('#TA_CALLS3','EVENT_DT','DTV','Ordered','All','Update Only','Offers_Applied_Lst_1D_DTV');
  call Decisioning_Procs.Add_Offers_Software('#TA_CALLS3','EVENT_DT','BB','Ordered','All','Update Only','Offers_Applied_Lst_1D_BB');
  call Decisioning_Procs.Add_Offers_Software('#TA_CALLS3','EVENT_DT','Line Rental','All','Ordered','Update Only','Offers_Applied_Lst_1D_LR');
  call Decisioning_Procs.Add_Offers_Software('#TA_CALLS3','EVENT_DT','MS','Ordered','All','Update Only','Offers_Applied_Lst_1D_MS');
  call Decisioning_Procs.Add_Offers_Software('#TA_CALLS3','EVENT_DT','HD','Ordered','All','Update Only','Offers_Applied_Lst_1D_HD');
  ...
  delete from Decisioning.ECONOMETRICS_TA_CALLS_ACCOUNT_LEVEL_beta6
    where WEEK_START between PERIOD_START and PERIOD_END;
  
  insert into Decisioning.ECONOMETRICS_TA_CALLS_ACCOUNT_LEVEL_beta6
    select ACCOUNT_NUMBER,
      COUNTRY,
      EVENT_DT,
      subscription_id,
      ...
      skyasia_Added,
      skyasia_Removed
      from #ECONOMETRICS_TA_CALLS_NEW;
	  
  delete from Decisioning.ECONOMETRICS_TA_CALLS_beta6
    where WEEK_START between PERIOD_START and PERIOD_END;
	
	
  insert into Decisioning.ECONOMETRICS_TA_CALLS_beta6
    select YEAR_WEEK,
      WEEK_START,
      COUNTRY,
      SAVED_FLAG,
  ...
  select ACCOUNT_NUMBER,
    ...
    PRE_CALL_TOTAL_SPORTS,
    PRE_CALL_TOTAL_MOVIES,
    PRE_CALL_BB_UNLIMITED,
    PRE_CALL_BB_LITE,
    PRE_CALL_BB_FIBRE_CAP,
    PRE_CALL_BB_FIBRE_UNLIMITED,
    POST_CALL_ORIGINAL,
    POST_CALL_VARIETY,
    POST_CALL_FAMILY,
    POST_CALL_SKYQ,
    ...
    POST_CALL_TOTAL_SPORTS,
    POST_CALL_TOTAL_MOVIES,
    POST_CALL_BB_UNLIMITED,
    POST_CALL_BB_LITE,
    POST_CALL_BB_FIBRE_CAP,
    POST_CALL_BB_FIBRE_UNLIMITED,
    ACTUAL_OFFER_STATUS,
    INTENDED_OFFER_STATUS,
    ANY_OFFER_Active,
    ANY_OFFER_GIVEN,
  ...
  delete from Decisioning.ECONOMETRICS_TA_CALLS_OFFERS_beta6
    where WEEK_START between PERIOD_START and PERIOD_END;
  insert into Decisioning.ECONOMETRICS_TA_CALLS_OFFERS_beta6
    select YEAR_WEEK,
      WEEK_START,
      COUNTRY,
      SAVED_FLAG,
  ...
  delete from Decisioning.ECONOMETRICS_TA_CALLS_OFFERS_ENDING_beta6
    where WEEK_START between PERIOD_START and PERIOD_END;
	
  insert into Decisioning.ECONOMETRICS_TA_CALLS_OFFERS_ENDING_beta6
    select distinct
      YEAR_WEEK,
      WEEK_START,
      COUNTRY,
  ...
  delete from Decisioning.ECONOMETRICS_TA_CALLS_OFFERS_ACTIVE_beta6
    where WEEK_START between PERIOD_START and PERIOD_END;
  insert into Decisioning.ECONOMETRICS_TA_CALLS_OFFERS_ACTIVE_beta6
    select distinct
      YEAR_WEEK,
      WEEK_START,
      COUNTRY,
...
call vha02.Update_Econometrics_TA_Events_beta(today()-20,today()-10);

call DBA.sp_DDL ('drop', 'view', 'CITeam', 'ECONOMETRICS_TA_CALLS_beta6');
call DBA.sp_DDL ('create', 'view', 'CITeam', 'ECONOMETRICS_TA_CALLS_beta6',  'select * from Decisioning.ECONOMETRICS_TA_CALLS_beta6');

call DBA.sp_DDL ('drop', 'view', 'CITeam', 'ECONOMETRICS_TA_CALLS_OFFERS_beta6');
call DBA.sp_DDL ('create', 'view', 'CITeam', 'ECONOMETRICS_TA_CALLS_OFFERS_beta6',  'select * from Decisioning.ECONOMETRICS_TA_CALLS_OFFERS_beta6');

call DBA.sp_DDL ('drop', 'view', 'CITeam', 'ECONOMETRICS_TA_CALLS_OFFERS_ENDING_beta6');
call DBA.sp_DDL ('create', 'view', 'CITeam', 'ECONOMETRICS_TA_CALLS_OFFERS_ENDING_beta6',  'select * from Decisioning.ECONOMETRICS_TA_CALLS_OFFERS_ENDING_beta6');

call DBA.sp_DDL ('drop', 'view', 'CITeam', 'ECONOMETRICS_TA_CALLS_OFFERS_ACTIVE_beta6');
call DBA.sp_DDL ('create', 'view', 'CITeam', 'ECONOMETRICS_TA_CALLS_OFFERS_ACTIVE_beta6',  'select * from Decisioning.ECONOMETRICS_TA_CALLS_OFFERS_ACTIVE_beta6');

call DBA.sp_DDL ('drop', 'view', 'CITeam', 'ECONOMETRICS_TA_CALLS_ACCOUNT_LEVEL_beta6');
call DBA.sp_DDL ('create', 'view', 'CITeam', 'ECONOMETRICS_TA_CALLS_ACCOUNT_LEVEL_beta6',  'select * from Decisioning.ECONOMETRICS_TA_CALLS_ACCOUNT_LEVEL_beta6');
