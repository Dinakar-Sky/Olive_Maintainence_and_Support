...
/*
Alter table Decisioning.PL_Entries_DTV rename PL_Entries_DTV_Old
...
begin
  ...
  select Account_Number,
    ...
    STATUS_CODE as Movement_Type, -- Movement Type
    STATUS_START_DT as Calendar_Date, -- Calendar Date
    ...
    case when cast(STATUS_START_DT as date)-cast(STATUS_END_DT as date) = 0 then 'Y' else 'N' end as Same_Day_Movement, -- Same Day Movement
    case when cast(PREV_STATUS_START_DT as date)-cast(STATUS_START_DT as date) = 0 then 'Y' else 'N' end as Same_Day_Prev_Status_Move, -- Same Day Prev Status Move
    ...
    or PC_To_AB > 0 then
      1 else 0 end as AB_Pending_Termination,
    --V_pending_terminations Includes intraday movements where customer in AB <24Hrs
    case when STATUS_CODE = 'PC' or Same_Day_Cancel = 1 then 1 else 0 end as Gross_PC_Pending_Cancellation, --V_pending_cancellations,
    case when Same_Day_Prev_Status_Move = 'Y' and Movement_Type = 'PO' and PREV_STATUS_CODE = 'PC' and STATUS_CODE = 'PO' then
      1 else 0 end as Same_Day_Cancel,
    --Includes intraday movements activating same day as cancel but excludes customers going direct from AC --> PO
    case when Same_Day_Prev_Status_Move = 'Y' and PREV_STATUS_CODE = 'PC' and STATUS_CODE = 'AC' then
      1 else 0 end as Same_Day_PC_Reactivation,
    case when Same_Day_Prev_Status_Move = 'Y' and PREV_STATUS_CODE = 'AB' and STATUS_CODE = 'AC' then
      1 else 0 end as Same_Day_AB_Reactivation,
  ...
  -----------------------------------------------------------------------------------------------------------------------------------------------------
  delete from Decisioning.PL_Entries_DTV as MoR
  ...
  update Decisioning.PL_Entries_DTV as MoR
    set Subs_Year = sc.subs_year,
    Subs_Quarter_Of_Year = sc.Subs_Quarter_Of_Year,
    Subs_Week_Of_Year = sc.Subs_Week_Of_Year,
    Subs_Week_And_Year = sc.Subs_Week_And_Year from
    Decisioning.PL_Entries_DTV as MoR
  ...
  select MoR.account_number,MoR.event_dt,
    ...
    row_number() over(partition by MoR.account_number,MoR.event_dt order by csh.effective_from_datetime desc,ZeroDuration asc,csh.effective_to_datetime asc) as PC_Rnk
    into #PC_Future_Effective_Dt
    from Decisioning.PL_Entries_DTV as MoR
  ...
  select MoR.account_number,
    MoR.event_dt,
    CSH.status_end_dt as PC_Effective_To_dt,
    cast(null as varchar(10)) as Next_Status_Code,
    case when csh.effective_to_dt = csh.effective_from_dt then 1 else 0 end as ZeroDuration,
    Row_number() over(partition by MoR.account_number,MoR.event_dt order by ZeroDuration asc,status_end_dt desc) as Status_change_rnk
  ...
  update #PC_Status_Change as MoR
    set Next_Status_Code = csh.status_code from
    #PC_Status_Change as MoR
    join cust_subs_hist as CSH
    on CSH.account_number = MoR.account_number
  ...
  select MoR.account_number,
    MoR.event_dt,
    CSH.status_end_dt as AB_Effective_To_dt,
    cast(null as varchar(10)) as Next_Status_Code,
    case when csh.effective_to_dt = csh.effective_from_dt then 1 else 0 end as ZeroDuration,
    Row_number() over(partition by MoR.account_number,MoR.event_dt order by ZeroDuration asc,status_end_dt desc) as Status_change_rnk
  ...
  update #AB_Status_Change as MoR
    set Next_Status_Code = csh.status_code from
    #AB_Status_Change as MoR
    join cust_subs_hist as CSH
    on CSH.account_number = MoR.account_number
  ...
  update Decisioning.PL_Entries_DTV
    ...
    when PC_Effective_To_Dt is not null and PC_Next_Status_Code <> 'AC' then 0 end,
    AB_Reactivation = case when AB_Effective_To_Dt is not null and AB_Next_Status_Code = 'AC' then 1
    when AB_Effective_To_Dt is not null and AB_Next_Status_Code <> 'AC' then 0 end
    where PC_Reactivation is null and AB_Reactivation is null;
  -----------------------------------------------------------------------------------------------------------------------------------------------------
  -- Update DTV_Churn Table
  -----------------------------------------------------------------------------------------------------------------------------------------------------
  delete from Decisioning.Churn_DTV as MoR
    where Event_Dt >= Refresh_dt;
  ...
  update Decisioning.Churn_DTV as MoR
    set Subs_Year = sc.subs_year,
    Subs_Quarter_Of_Year = sc.Subs_Quarter_Of_Year,
    Subs_Week_Of_Year = sc.Subs_Week_Of_Year,
    Subs_Week_And_Year = sc.Subs_Week_And_Year from
    Decisioning.Churn_DTV as MoR
    join sky_calendar as sc
    on sc.calendar_date = MoR.Event_dt
    where MoR.Subs_Week_And_Year is null;
  commit work
