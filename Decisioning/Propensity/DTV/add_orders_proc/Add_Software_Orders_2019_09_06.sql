...
GO
DROP PROCEDURE if exists Decisioning_procs.Add_Software_Orders
GO
...
begin
  ...
  set prod_type
	...
	when str_replace(upper(prod_type),' ','_') in('BOXSETS' ) then 'BOXSETS' 
	when str_replace(upper(prod_type),' ','_') in('UOD' ) then 'UOD' 
...
GO
GRANT EXECUTE ON Decisioning_procs.Add_Software_Orders TO public;