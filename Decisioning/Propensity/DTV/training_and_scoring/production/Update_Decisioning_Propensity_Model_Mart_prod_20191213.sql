...
if Propensity_Table = 'Scoring' then
  ...
  drop table if exists #uf_mapping;

  select account_number, base_dt, sum(uf_flag) as uf_flag
  into #uf_mapping
  from ( select a.account_number, a.base_dt,
          case when g.uprn is not null
            and g.date_available_on_emlc in ('Live', 'Commissioned')
            and s.l2s_actual_date <= a.base_dt
          then 1 else 0 end as uf_flag
        from Decisioning.PROPENSITY_MODEL_MART_SCORING a
        left join ORD_SURVEY_DPA b
        on a.cb_key_household = b.cb_key_household
        left join STRATEGIC_ACCESS_G_FAST g
        on cast(b.UPRN as double) = cast(g.UPRN as double)
        left join STRATEGIC_ACCESS_SNS_CABLELINK s
        on s.l2s_id = g.l2s 
        group by a.account_number, a.base_dt, uf_flag
        ) as base
  group by account_number, base_dt;

update Decisioning.Propensity_Model_Mart_Scoring
  set Exchange_Name = case when b.uf_flag=1 then 'Yes' else 'No' end
  from Decisioning.Propensity_Model_Mart_Scoring a
  left join #uf_mapping b
  on a.account_number = b.account_number and a.base_dt = b.base_dt;
