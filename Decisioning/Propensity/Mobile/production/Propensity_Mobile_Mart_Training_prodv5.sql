...
dba.sp_drop_table 'Decisioning','Propensity_Mobile_Mart_Training'
dba.sp_create_table 'Decisioning','Propensity_Mobile_Mart_Training', 
...
drop table if exists 	Propensity_Mobile_Mart_Training;	
create table Propensity_Mobile_Mart_Training		
	(	
 	base_dt date  default	null,
 	mobile_account_number varchar(20) default null,	
 	portfolio_id varchar(50) default null,
...
GO
DROP PROCEDURE if exists Decisioning_procs.Update_Decisioning_Propensity_Mobile_Mart_Training
GO

create procedure Decisioning_procs.Update_Decisioning_Propensity_Mobile_Mart_Training(period_start date default null,period_end date default null ) 
...
set period_start = '2017-07-01';
set period_end = today() -1;
*/
if period_start is null then
      set period_start = (select max(base_dt) from Decisioning.Propensity_Mobile_Mart_Training)
end if;
...
select cast(base_Dt-day(calendar_date)+1 as date) as  start_dt, cast(calendar_date as date) as base_Dt,
	      'GBP' as currency_code 
into #weeks_uk
from sky_calendar 
where datepart(day,calendar_date+1) = 1
and calendar_date between period_start and period_end + 31;
...
from sky_calendar 
where datepart(day,calendar_date+1) = 1
and calendar_date between period_start and period_end;
...
from 
CUST_SINGLE_MOBILE_VIEW_HIST
where cb_data_date > period_start;
...
from Cust_single_mobile_view_hist
group by  account_number, igor_trans_date, status_code
having end_date <= (select max(base_dt) from #mobile_base);   --edit to remove duplicates
...
, sum(a.igor_redeemed_value_mb/1024) OVER(PARTITION BY a.account_number  ORDER BY a.account_number,a.igor_trans_date, a.end_date ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) AS igor_redemption_cum_sum
into #igor_redemptions2
from #igor_redemptions a
inner join #weeks as b
on  a.end_date between b.start_dt and b.base_dt
where b.currency_code = 'GBP';
...
from #mobile_base  base 
left join #igor_redemptions2 add_info 
on base.mobile_account_number = add_info.account_number 
and  base.base_dt = add_info.base_dt -- between add_info.start_date and add_info.end_date 
and add_info.status_code in('AC','R','CRQ','BCRQ');
...
select account_number
, month(cb_data_date) as month
, year(cb_data_date) as year
...
update #mobile_base
	...
	and month(base.base_dt) = usage.month and year(base.base_dt) = usage.year;
...
delete from Decisioning.Propensity_Mobile_Mart_Training
where base_dt between period_start and period_end;


insert into Decisioning.Propensity_Mobile_Mart_Training
select 
...
GO
GRANT EXECUTE ON Decisioning_procs.Update_Decisioning_Propensity_Mobile_Mart_Training TO public;


/*

call Decisioning_procs.Update_Decisioning_Propensity_Mobile_Mart_Training();

View 

call DBA.sp_DDL ('drop', 'view', 'CITeam', 'Propensity_Mobile_Mart_Training');
call DBA.sp_DDL ('create', 'view', 'CITeam', 'Propensity_Mobile_Mart_Training',  'select * from Decisioning.Propensity_Mobile_Mart_Training');
