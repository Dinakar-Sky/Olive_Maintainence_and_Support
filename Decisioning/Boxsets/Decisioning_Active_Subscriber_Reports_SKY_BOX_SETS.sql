...
GO
DROP PROCEDURE if exists Decisioning_Procs.Update_Active_Subscriber_Report_SKY_BOX_SETS
GO
create procedure Decisioning_Procs.Update_Active_Subscriber_Report_SKY_BOX_SETS( Refresh_Dt date default null ) 
...
begin
  ...
  */
  ...
  if Refresh_Dt is null then
    set Refresh_dt = (select max(effective_from_dt)-4*7 from Decisioning.Active_Subscriber_Report_Src where subscription_sub_type = 'SKY_BOX_SETS');
  ...
  insert into #Subs_Movements
    select ph_subs_hist_sk,
      ...
      'ENHANCED' as Subscription_Type,
      Current_Product_sk,
      'SKY_BOX_SETS' as Subscription_Sub_Type,
      Effective_From_Dt,
      Effective_To_Dt,
      Effective_From_Datetime,
      cast(null as integer) as Effective_From_Subs_Week,
...
delete from #Subs_Movements where Product_Holding <> 'Sky Box Sets';
  ...
  select asr.owning_cust_account_id,asr.subscription_sub_type,asr.effective_from_dt as Change_Dt
    ...
    from Decisioning.Active_Subscriber_Report_Src as asr
      ...
      and asr_2.Subscription_Sub_Type = 'SKY_BOX_SETS'
      and asr_2.Active_Subscriber = 1
	where asr.Effective_To_Dt >= Refresh_Dt
    and asr.Subscription_Sub_Type = 'SKY_BOX_SETS'
  ...
  select distinct Substr(current_product_description,15,200) as Sky_BB_Type
    into #Sky_BB_Types
    from cust_subs_hist
    where subscription_sub_type = 'SKY_BOX_SETS';
  select Owning_Cust_Account_ID,ph_subs_hist_sk,Effective_From_Dt,Subscription_Sub_Type,
    ...
    where effective_to_dt >= Refresh_Dt
    and subscription_sub_type in('SKY_BOX_SETS');
  commit work;
...
GO
GRANT EXECUTE ON Decisioning_Procs.Update_Active_Subscriber_Report_SKY_BOX_SETS TO public;

/*
call Decisioning_Procs.Update_Active_Subscriber_Report_SKY_BOX_SETS('2012-01-01');
