/*
*********************************************************************************
** File: Churn_Boxsets.sql
** Name: Churn_Boxsets
** Desc: Code for calculating Churn for "Boxsets"
...
dba.sp_drop_table 'Decisioning','Churn_Boxsets'
dba.sp_create_table 'Decisioning','Churn_Boxsets',
...
Call Decisioning_Procs.Update_Decisioning_Churn_Boxsets()

Select top 1000 * from Decisioning.Churn_Boxsets

Drop procedure if exists Decisioning_Procs.Update_Decisioning_Churn_Boxsets;
Create procedure Decisioning_Procs.Update_Decisioning_Churn_Boxsets(Refresh_Dt date default null)
...
BEGIN 
Set Refresh_Dt = (Select max(event_dt) - 2*7 from Decisioning.Churn_Boxsets);
END
End If;

Delete from Decisioning.Churn_Boxsets where Event_Dt >= Refresh_Dt;

DROP TABLE IF EXISTS #Churn_Boxsets_Temp;
...
,CASE 
    ...
    WHEN Boxsets_EoD.account_number IS NULL THEN 'Product Churn'
END AS Churn_Type
    ,CASE
...
LEFT JOIN cust_subs_hist Boxsets_SoD ON Boxsets_SoD.account_number = csh.account_number
    AND csh.status_start_dt BETWEEN Boxsets_SoD.effective_from_dt + 1 AND Boxsets_SoD.effective_to_dt
    AND Boxsets_SoD.subscription_sub_type = 'SKY_BOX_SETS'
    AND Boxsets_SoD.status_code IN ('AC', 'AB', 'PC')

LEFT JOIN cust_subs_hist Boxsets_EoD -- Active MS subs at start of day
    ON Boxsets_EoD.account_number = csh.account_number
    AND csh.status_start_dt BETWEEN Boxsets_EoD.effective_from_dt AND Boxsets_EoD.effective_to_dt - 1
    AND Boxsets_EoD.subscription_sub_type = 'SKY_BOX_SETS'
    AND Boxsets_EoD.status_code IN ('AC', 'AB', 'PC')
...
WHERE 
    ...
    AND Boxsets_SoD.account_number IS NOT NULL
    AND Boxsets_EoD.account_number IS NULL ;
        
----------------------------------------------------------------------------------------------
-- Add Subscriber Level flag to reoncile the numbers at account level

DROP TABLE IF EXISTS #Churn_Boxsets;
...
FROM (
    ...
    FROM #Churn_Boxsets_Temp
) FinalChurn_SGE;
----------------------------------------------------------------------------------------------
-- Insert into final table
INSERT INTO Decisioning.Churn_Boxsets (
...
Grant Execute on Decisioning_Procs.Update_Decisioning_Churn_Boxsets to Decisioning;

/* ======================================== Queries for Testing ========================================
call Decisioning_Procs.Update_Decisioning_Activations_DTV('2017-10-01');
select top 10 * from Decisioning.Churn_Boxsets;
select count(*) from Decisioning.Churn_Boxsets;
====================================================================================================== */
