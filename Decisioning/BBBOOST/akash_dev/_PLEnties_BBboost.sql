/*
*********************************************************************************
** File: PL_Entries_BBBOOST.sql
** Name: PL_Entries_BBBOOST
** Desc: Code for calculating PLEntires  for "BBBOOST"
** Auth: Fractal Team 
** Date: 17/11/2017
************************************************************************************
****                                                            Change History                                                          ****
...
** --           ---------       -------------                   ------------------------------------
** 1            17/11/2017      PL_Entries_BBBOOST              First commit
** 2        	03/01/2018  	Vikram Haibate          Add DDL and Procs 
** 3 		14/03/2019	Dinakar Karanam		BBBOOST
**********************************************************************************


dba.sp_drop_table 'CITeam','PL_Entries_BBBOOST_beta'
dba.sp_create_table 'CITeam','PL_Entries_BBBOOST_beta',
...
/* ================================================================================= */
DROP TABLE IF EXISTS aks03.PL_Entries_BBBOOST_beta
CREATE TABLE aks03.PL_Entries_BBBOOST_beta (
Subs_Year int  default      null,
Subs_Week_Of_Year int  default      null,
Subs_Week_And_Year int  default     null,
event_dt date  default      null,
...
GO 
-- Drop procedure if exists Decisioning_Procs.Update_Decisioning_PL_Entries_BBBOOST;
Drop procedure if exists aqi02.Update_Decisioning_PL_Entries_BBBOOST;
GO 
-- Create procedure Decisioning_Procs.Update_Decisioning_PL_Entries_BBBOOST(Refresh_Dt date default null)
Create procedure aqi02.Update_Decisioning_PL_Entries_BBBOOST(Refresh_Dt date default null)
...
BEGIN 
-- Set Refresh_Dt = (Select max(event_dt) - 2*7 from Decisioning.PL_Entries_BBBOOST);
Set Refresh_Dt = '2019-09-01';
END
End If;

-- Delete from Decisioning.PL_Entries_BBBOOST where Event_Dt >= Refresh_Dt;
Delete from CITeam.PL_Entries_BBBOOST_beta where Event_Dt >= Refresh_Dt;
...
SELECT   
        ...
        ,CURRENT_PRODUCT_DESCRIPTION AS Product_Holding
...
WHERE  
                csh.effective_from_dt >= '2019-07-01'
...
GROUP BY
        ...
        ,Country
        ,SUBSCRIPTION_SUB_TYPE --Subscription Sub Type
        ,CURRENT_PRODUCT_DESCRIPTION -- CPD
        ,CURRENCY_CODE -- Currency Code
        ,STATUS_CODE -- Status Code
        ,PREV_STATUS_CODE -- Prev Status Code
...
--------------------------------------------------------------------------------------------------------------
-- select top 1000 * from #MoR_Subs_Events
COMMIT;
...
-- --------------------------------------------------------------------------------------------------------------
DROP TABLE IF EXISTS #PL_Entries_BBBOOST;

-- INSERT INTO #PL_Entries_BBBOOST (Subs_Year, Subs_Week_Of_Year, Subs_Week_And_Year, Event_Dt, account_number, Country,
...
ON csh.account_number = MoR.account_number
    ...
    AND csh.status_end_dt >= '2019-01-01' 
...
-- INSERT INTO Decisioning.PL_Entries_BBBOOST 
INSERT INTO CITeam.PL_Entries_BBBOOST_beta 
(Subs_Year,
Subs_Week_Of_Year,
Subs_Week_And_Year,
event_dt,
...
-- FROM #PL_Entries_BBBOOST PL LEFT JOIN Decisioning.Activations_BBBOOST ACT
FROM #PL_Entries_BBBOOST PL LEFT JOIN CITeam.Activations_BBBOOST_beta ACT
...
END;
GO
-- Grant Execute on Decisioning_Procs.Update_Decisioning_PL_Entries_BBBOOST to Decisioning;
Grant Execute on aks03.Update_Decisioning_PL_Entries_BBBOOST to public;

call aks03.Update_Decisioning_PL_Entries_BBBOOST('2019-09-01');

/* ======================================== Queries for Testing ========================================
call Decisioning_Procs.Update_Decisioning_PL_Entries_BBBOOST('2019-01-01');
SELECT TOP 1000 * FROM Decisioning.PL_Entries_BBBOOST;
select count(*) from Decisioning.PL_Entries_BBBOOST;



call DBA.sp_DDL ('drop', 'view', 'CITeam', 'PL_Entries_BBBOOST');
call DBA.sp_DDL ('create', 'view', 'CITeam', 'PL_Entries_BBBOOST',  'select * from Decisioning.PL_Entries_BBBOOST');
====================================================================================================== */
select distinct Product_Holding from  CITeam.PL_Entries_BBBOOST_beta 