...
DROP PROCEDURE if exists aks03.Update_Active_Subscriber_Report_BBBOOST;
GO
-- create procedure Decisioning_Procs.Update_Active_Subscriber_Report_BBBOOST( Refresh_Dt date default null ) 
create procedure aks03.Update_Active_Subscriber_Report_BBBOOST( Refresh_Dt date default null ) 
  ...
  */
  ...
  insert into #Subs_Movements
    select ph_subs_hist_sk,
      ...
      where csh.effective_to_dt >= '2019-07-01'
...
delete from #Subs_Movements where Product_Holding not like '%Sky Broadband Boost%';
  ...
  delete from #Subs_Movements where effective_to_dt < '2019-07-01';
  
  -- delete from Decisioning.Active_Subscriber_Report_Src
  delete from aqi02.ASR_Src_beta
    where Effective_To_Dt >= '2019-07-01'
    and subscription_sub_type = any(select distinct subscription_sub_type from #Subs_Movements);
 -- insert into aqi02.ASR_Src_beta select * from  Decisioning.Active_Subscriber_Report_Src where 
-- select distinct product_holding from Decisioning.Active_Subscriber_Report_Src
-- describe Decisioning.Active_Subscriber_Report_Src
-- drop table   #Effective_To
select sm.ph_subs_hist_sk,sm.effective_to_dt,max(asr.effective_to_dt) as effective_from_dt
    ...
    from #Subs_Movements as sm
      ...
      --join aks03.Active_Subscriber_Report_Src as asr
      on asr.ph_subs_hist_sk = sm.ph_subs_hist_sk
      and sm.subscription_sub_type = asr.subscription_sub_type
      and asr.effective_to_dt < '2019-07-01'
      and '2019-07-01' between sm.effective_from_dt+1 and sm.effective_to_dt
...
--select * from aqi02.ASR_Src_beta from Decisioning.Active_Subscriber_Report_Src where 1=0
  ...
  insert into aqi02.ASR_Src_beta_1( Effective_From_Dt,Effective_To_Dt,Account_Number,Account_Type,Account_Sub_Type,Owning_Cust_Account_ID,Subscription_Type,Subscription_Sub_Type,
  ...
  select asr.owning_cust_account_id,asr.subscription_sub_type,asr.effective_from_dt as Change_Dt
    ...
    from aks03.Active_Subscriber_Report_Src as asr
      -- join Decisioning.Active_Subscriber_Report_Src as asr_2
      join aks03.Active_Subscriber_Report_Src as asr_2
  ...
  while(select count() from #Active_Subs_Change_Dts) > 0 loop
    ...
    select sm.*
      ...
      from aks03.Active_Subscriber_Report_Src as sm
    ...
    update aks03.Active_Subscriber_Report_Src as sm
    ...
    insert into aks03.Active_Subscriber_Report_Src
...
GRANT EXECUTE ON aks03.Update_Active_Subscriber_Report_BBBOOST TO public;

call aks03.Update_Active_Subscriber_Report_BBBOOST('2012-01-01'); 
GRANT select on aks03.Active_Subscriber_Report_Src to public;
/*
...
create table aqi02.ASR_Src_beta_1(
Effective_From_Dt date,
Effective_To_Dt date default null,
Effective_From_Subs_Week integer default null,
Effective_To_Subs_Week integer default null,
...
Active_Subscriber tinyint default null,
Active_Subscription tinyint default null,
PH_Subs_Status_Start_Dt date default null,
PH_Subs_Status_End_Dt_Dt date default null,
PH_Subs_Prev_Status_Code varchar(10) default null,
PH_Subs_Status_Reason varchar(255) default null)
