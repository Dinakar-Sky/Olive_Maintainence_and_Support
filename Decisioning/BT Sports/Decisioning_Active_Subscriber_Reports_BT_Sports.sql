...
-- call Decisioning_procs.Update_Active_Subscriber_Report_BT_SPORTS('2019-07-01')
setuser Decisioning_Procs;
GO
DROP PROCEDURE if exists Decisioning_procs.Update_Active_Subscriber_Report_BT_SPORTS
GO
create procedure Decisioning_procs.Update_Active_Subscriber_Report_BT_SPORTS( Refresh_Dt date default null ) 
...
begin
  ...
  */
  ...
  if Refresh_Dt is null then
    set Refresh_dt = (select max(effective_from_dt)-4*7 from Decisioning.Active_Subscriber_Report_Src where subscription_sub_type in 
('Sport Extra','Sport Extra HD'));
...
drop table if exists #Subs_Movements;
  ...
  insert into #Subs_Movements
    select ph_subs_hist_sk,
      ...
      and upper(csh.Subscription_Sub_Type) in ('SPORT EXTRA','SPORT EXTRA HD')
...
-- delete from #Subs_Movements where Product_Holding <> 'Sky Kids';
	 drop table if exists #Inactive_Subs; 
  ...
  select asr.owning_cust_account_id,asr.subscription_sub_type,asr.effective_from_dt as Change_Dt
    ...
    from Decisioning.Active_Subscriber_Report_Src as asr
      ...
      and asr_2.Subscription_Sub_Type in ('Sport Extra','Sport Extra HD')
      and asr_2.Active_Subscriber = 1
	where asr.Effective_To_Dt >= Refresh_Dt
    and upper(asr.Subscription_Sub_Type)  in ('SPORT EXTRA','SPORT EXTRA HD')
  ...
  select asr.owning_cust_account_id,asr.subscription_sub_type,asr.effective_to_dt as Change_Dt
    from Decisioning.Active_Subscriber_Report_Src as asr
      ...
      and asr_2.Active_Subscriber = 1
    where asr.Effective_To_Dt >= Refresh_Dt
    and asr.Subscription_Sub_Type in ('Sport Extra','Sport Extra HD')
    and asr.Active_Subscriber = 1
  ...
  select distinct Substr(current_product_description,15,200) as Sky_BB_Type
    into #Sky_BB_Types
    from cust_subs_hist
    where subscription_sub_type in ('Sport Extra','Sport Extra HD');
...
GO
GRANT EXECUTE ON Decisioning_procs.Update_Active_Subscriber_Report_BT_SPORTS TO public;

/*
call Decisioning_procs.Update_Active_Subscriber_Report_BT_SPORTS('2019-08-01');

*/
/*
Decisioning.Active_Subscriber_Report_Src_BT_SPORTS_beta


grant all on aqi02.orders_daily_bt to public;
grant all on aqi02.orders_detail_bt to public;
grant all on Decisioning.Active_Subscriber_Report_Src  to public;



select top 10 * from CITeam.Pricing_Table where pro
...
select top 10 * from Decisioning.Active_Subscriber_Report_Src

select * into Decisioning.Active_Subscriber_Report_Src from 
Decisioning.Active_Subscriber_Report_Src
where 1=0
