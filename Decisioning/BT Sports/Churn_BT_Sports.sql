/*
*********************************************************************************
** File: Churn_BT_Sports.sql
** Name: Churn_BT_Sports
** Desc: Code for calculating Churn for "BT_Sports"
** Date: 16/08/2019
************************************************************************************
****                                Change History                              ****
************************************************************************************
...
/*
dba.sp_drop_table 'Decisioning','Churn_BT_Sports'
dba.sp_create_table 'Decisioning','Churn_BT_Sports',
...
GO
Drop procedure if exists Decisioning_Procs.Update_Decisioning_Churn_BT_Sports;
GO
Create procedure Decisioning_Procs.Update_Decisioning_Churn_BT_Sports(Refresh_Dt date default null) 
...
BEGIN 
Set Refresh_Dt = (Select max(event_dt) - 2*7 from Decisioning.Churn_BT_Sports);
END
End If;

Delete from Decisioning.Churn_BT_Sports where Event_Dt >= Refresh_Dt;

DROP TABLE IF EXISTS #Churn_BT_Sports_Temp;
...
,cast(csh.Subscription_ID as varchar(80)),csh.status_code AS Movement_Type
...
,CASE 
    ...
    WHEN BT_Sports_EoD.account_number IS NULL THEN 'Product Churn'
END AS Churn_Type
,CASE
...
LEFT JOIN cust_subs_hist BT_Sports_SoD ON BT_Sports_SoD.account_number = csh.account_number
    AND csh.status_start_dt BETWEEN BT_Sports_SoD.effective_from_dt + 1 AND BT_Sports_SoD.effective_to_dt
    AND BT_Sports_SoD.subscription_sub_type in ('Sport Extra','Sport Extra HD')
    AND BT_Sports_SoD.status_code IN ('AC', 'AB', 'PC')
    
LEFT JOIN cust_subs_hist BT_Sports_EoD -- Active MS subs at start of day
    ON BT_Sports_EoD.account_number = csh.account_number
    AND csh.status_start_dt BETWEEN BT_Sports_EoD.effective_from_dt AND BT_Sports_EoD.effective_to_dt - 1
    AND BT_Sports_EoD.subscription_sub_type in ('Sport Extra','Sport Extra HD')
    AND BT_Sports_EoD.status_code IN ('AC', 'AB', 'PC')
...
WHERE 
    ...
    AND BT_Sports_SoD.account_number IS NOT NULL
    AND BT_Sports_EoD.account_number IS NULL
    ;

-- Add Subscriber Level flag to reoncile the numbers at account level

DROP TABLE IF EXISTS #Churn_BT_Sports;
...
FROM (
    ...
    FROM #Churn_BT_Sports_Temp
) FinalChurn_BT_Sports;


-- Insert into final table
INSERT INTO Decisioning.Churn_BT_Sports (
...
END;
GO
Grant Execute on Decisioning_Procs.Update_Decisioning_Churn_BT_Sports to public;


/* ======================================== Queries for Testing ========================================
call Decisioning_Procs.Update_Decisioning_Churn_BT_Sports('2017-10-01');
select top 100 * from Decisioning.Churn_BT_Sports;
select count(*) from Decisioning.Churn_BT_Sports;
====================================================================================================== */
